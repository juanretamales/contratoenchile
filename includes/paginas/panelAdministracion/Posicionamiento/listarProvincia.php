		...
		<div id="error"></div>
			...
			<?php
							...
							for($i=0;$i<count($provincias);$i++)
							{
								?>
				<tr>
					<td><?php echo $provincias [$i] ['id_prov']; ?></td>
					<td><?php echo $provincias [$i] ['nom_prov']; ?></td>
					<td><?php 
								$arg = array ('id_reg'=>$provincias [$i] ['id_reg']);
								$region=listarRegion($arg);
								echo $region [0] ['nom_reg'];
																?></td>
					<td><a onclick="modificar(<?php echo $provincias [$i] ['id_prov']; ?>)">
						<img title="Modificar" width="20px" src="<?php echo WEB_BASE;?>imagenes/UI/modificar.png">
					</a></td>
					<td><a onclick="eliminarProvincia(<?php echo $provincias [$i] ['id_prov']; ?>, '<?php echo $provincias [$i] ['nom_prov']; ?>')">
