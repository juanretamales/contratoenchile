		...
		<div id="error"></div>
			...
			<?php
							...
							for($i=0;$i<count($comunas);$i++)
							{
								?>
				<tr>
					<td><?php echo $comunas [$i] ['id_com']; ?></td>
					<td><?php echo $comunas [$i] ['nom_com']; ?></td>
					<td><?php 
								$arg = array ('id_prov'=>$comunas [$i] ['id_prov']);
								$provincia=listarProvincia($arg);
								echo $provincia [0] ['nom_prov'];
																?></td>
					<td><a onclick="modificar(<?php echo $comunas [$i] ['id_com']; ?>)">
						<img title="Modificar" width="20px" src="<?php echo WEB_BASE; ?>imagenes/UI/modificar.png">
					</a></td>
					<td><a onclick="eliminarComuna(<?php echo $comunas [$i] ['id_com']; ?>, '<?php echo $comunas [$i] ['nom_com']; ?>')">
