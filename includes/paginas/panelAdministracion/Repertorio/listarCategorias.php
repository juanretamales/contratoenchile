		...
		<div id="error"></div>
			...
			<?php
							...
							for($i=0;$i<count($cat);$i++)
							{
								?>
				<tr>
					<td><?php echo $cat [$i] ['id_cat']; ?></td>
					<td><?php echo $cat [$i] ['nom_cat']; ?></td>
					<td><a onclick="modificar(<?php echo $cat [$i] ['id_cat']; ?>)">
						<img title="Modificar" width="20px" src="<?php echo WEB_BASE; ?>imagenes/UI/modificar.png">
					</a></td>
					<td><a onclick="eliminarCategoria(<?php echo $cat [$i] ['id_cat']; ?>, '<?php echo $cat [$i] ['nom_cat']; ?>')">
