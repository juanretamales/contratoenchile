...
if(isset($_REQUEST['pagina']))
{
	...
	<section id="contenido" >
			...
			<h1 class="titulo">Tipo Servicio</h1>
		...
		<div id="error"></div>
			...
			<?php
							...
							for($i=0;$i<count($ts);$i++)
							{
								?>
				<tr>
					<td><?php echo $ts [$i] ['id_ts']; ?></td>
					<td><?php echo $ts [$i] ['nom_ts']; ?></td>
					<td><a onclick="modificar(<?php echo $ts [$i] ['id_ts']; ?>)">
						<img title="Modificar" width="20px" src="<?php echo WEB_BASE; ?>imagenes/UI/modificar.png">
					</a></td>
					<td><a onclick="eliminarTiposervicio(<?php echo $ts [$i] ['id_ts']; ?>, '<?php echo $ts [$i] ['nom_ts']; ?>')">
