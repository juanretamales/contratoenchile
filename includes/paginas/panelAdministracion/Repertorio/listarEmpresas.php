	...
	<section id="contenido" >
			...
			<?php
							...
							for($i=0;$i<count($entidad);$i++)
							{
								?>
				<tr>
					<td><?php echo $entidad [$i] ['rut_sii']; ?></td>
					<td><?php echo $entidad [$i] ['nom_ent']; ?></td>
					<td><?php if($entidad [$i] ['subscripcion']>date( "Y-m-j" , time()))
					{ echo 'Si, hasta '.dateDecode($entidad [$i] ['subscripcion']);  } else { echo 'No'; }?></td>
					<td><label><?php echo $entidad [$i] ['email_ent']; ?></label></td>
					<td><?php echo $entidad [$i] ['tel_ent']; ?></td>
					<td><a onclick="modificar(<?php echo $entidad [$i] ['id_ent']; ?>)">
						<img title="Modificar" width="20px" src="<?php echo WEB_BASE; ?>imagenes/UI/modificar.png">
					</a></td>
					<td><a onclick="eliminarEmpresa(<?php echo $entidad [$i] ['id_ent']; ?>, '<?php echo $entidad [$i] ['nom_ent']; ?>')">
