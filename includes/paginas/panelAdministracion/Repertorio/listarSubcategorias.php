		...
		<div id="error"></div>
			...
			<?php
							...
							for($i=0;$i<count($scat);$i++)
							{
								?>
				<tr>
					<td><?php echo $scat [$i] ['id_scat']; ?></td>
					<td><?php echo $scat [$i] ['nom_scat']; ?></td>
					<td><?php
					for($j=0;$j<count($cat);$j++)
					{
						if($scat [$i] ['id_cat']==$cat [$j] ['id_cat'])
						{
							echo $cat [$j] ['nom_cat'];
							break;
						}
					...
					?></td>
					<td><a onclick="modificar(<?php echo $scat [$i] ['id_scat']; ?>)">
						<img title="Modificar" width="20px" src="<?php echo WEB_BASE; ?>imagenes/UI/modificar.png">
					</a></td>
					<td><a onclick="eliminarSubcategoria(<?php echo $scat [$i] ['id_scat']; ?>, '<?php echo $scat [$i] ['nom_scat']; ?>')">
