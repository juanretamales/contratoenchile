...
if(isset($_REQUEST['pagina']))
{
	...
	<section id="contenido" >
			...
			?>
				<tr>
					<td><?php echo $plan [$i] ['nom_plan']; ?></td>
					<td><?php echo $plan [$i] ['valor_plan']; ?></td>
					<td><?php echo $plan [$i] ['dias']; ?></td>
					<td><?php
					for($j=0;$j<count($est);$j++)
					{
						if($plan [$i] ['id_est']==$est [$j] ['id_est'])
						{
							echo $est [$j] ['nom_est'];
							break;
						}
					...
					?></td>
					<td><a onclick="modificar(<?php echo $plan [$i] ['id_plan']; ?>)">
						<img title="Modificar" width="20px" src="<?php echo WEB_BASE;?>imagenes/UI/modificar.png">
					</a></td>
					<td><a onclick="eliminarPlan(<?php echo $plan [$i] ['id_plan']; ?>, '<?php echo $plan [$i] ['nom_plan']; ?>')">
