...
if(isset($_REQUEST['pagina']))
{
	...
	<section id="contenido" >
			...
			?>
				<tr>
					<td>#<?php echo $bol [$i] ['id_bol']; ?></td>
					<td><?php echo dateDecode($bol [$i] ['fecha_bol']); ?></td>
					<td><?php
					for($j=0;$j<count($ent);$j++)
					{
						if($bol [$i] ['id_ent']==$ent [$j] ['id_ent'])
						{
							echo $ent [$j] ['nom_ent'];
							break;
						}
					...
					{
						if($bol [$i] ['id_plan']==$plan [$j] ['id_plan'])
						{
							echo $plan [$j] ['nom_plan'];
							break;
						}
					}
					?></td>
					<td><?php echo $bol [$i] ['monto']; ?></td>
					<td><?php
					for($j=0;$j<count($est);$j++)
					{
						if($bol [$i] ['id_ent']==$est [$j] ['id_est'])
						{
							echo $est [$j] ['nom_est'];
							break;
						}
					...
					<td><a onclick="modificar(<?php echo $bol [$i] ['id_bol']; ?>)">
						<img title="Modificar" width="20px" src="<?php echo WEB_BASE;?>imagenes/UI/modificar.png">
					</a></td>
					<td><a onclick="eliminarBoleta(<?php echo $bol [$i] ['id_bol']; ?>, '<?php echo $bol [$i] ['nom_plan']; ?>')">
